...
def takepenality(request): 
    ...
    return render(request, 'subcity/template/penalty/penality.html',{'trip_no':toal_trip,'penalty_type':penalty_status})
def showpenality(request): 
    show = penalty.objects.all()
    context = {'show': show}
    return render(request, 'subcity/template/penalty/penality.html', context) 
"""       
